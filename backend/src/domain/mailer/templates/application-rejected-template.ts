...
export const getApplicationRejectedTemplate = (
  ...
  const title = 'Actualización sobre tu aplicación';
  const statusColor = '#e53e3e';
  const messageHtml = `
      <p>Queremos agradecerte por tu interés en el puesto de <strong>${payload.jobPosition}</strong> y por el tiempo invertido en el proceso.</p>
      <div class="status-box" style="border-left-color: ${statusColor};">
        <p><strong>Estado:</strong> No seleccionado</p>
      </div>
      <p>En esta ocasión hemos decidido avanzar con otros candidatos cuyo perfil se ajusta más a nuestras necesidades actuales. Mantendremos tu perfil en nuestra base de datos para futuras oportunidades.</p>
    `;
